...
const long FFQFullSpec::ID_RESETBTN = wxNewId();
//  *)
...
const uint8_t NUM_BOOLEANS = 3;
const wxString BOOLEANS[3][2] = { {"0", "1"}, {"no", "yes"}, {"false", "true"} };

//---------------------------------------------------------------------------------------

bool GetBoolean(wxString val, bool def = false)
{
    for (uint8_t i = 0; i < NUM_BOOLEANS; i++)
    {
        if (val == BOOLEANS[0][0]) return false;
        else if (val == BOOLEANS[0][1]) return true;
    }
    return false;
}
//---------------------------------------------------------------------------------------

wxString GetCmdToken(wxString &from, bool is_name, bool is_composite, wxUniChar *sep)
{
   size_t p = 0;
   ...
   while (p < from.Len())
   {
       ...
       if (is_name)
       {
           ...
           //Extract the name part ending with equal or space
           if ((cc == EQUAL) || (cc == SPACE))
           {
               if (sep != nullptr) *sep = cc;
               break;
           }
           else res += cc;
       ...
       else if (is_composite)
       {
           ...
           //Extract a composite value terminated with colon
           if (cc == COLON)
           {
               if (sep != nullptr) *sep = cc;
               break;
       ...
       {
           //Extract a quoted or un-quoted value terminated with a space
           if (cc == quote) quote = 0;
           else if ((cc == QUOTE) || (cc == DQUOTE)) quote = cc;
           else if ((quote == 0) && (cc == SPACE))
           {
               if (sep != nullptr) *sep = cc;
               break;
           }
       ...
       //The old method could not handle spaces in composite
       /*if ((!quote) && ((cc == SPACE) || (cc == COLON) || (cc == EQUAL)))
       {
           if (sep != nullptr) *sep = cc;
           break;
       }
...
wxString GetFieldValue(FULLSPEC_FILE &file, FULLSPEC_FIELD &field)
{
    ...
    if (field.ctrl == nullptr)
    {

        //Get the value of a checkbox
        if (field.checked != field.check->IsChecked())
        {

            if (field.checked)
            {

                //Checked as default, guess the opposite of file.checkval
                for (uint8_t i = 0; (i < NUM_BOOLEANS) && (res.Length() == 0); i++)
                {
                    if (file.checkval == BOOLEANS[i][0]) res = BOOLEANS[i][1];
                    else if (file.checkval == BOOLEANS[i][1]) res = BOOLEANS[i][0];
                }

                #ifdef DEBUG
                if (res.Length() == 0) FFQConsole::Get()->AppendLine("FFQFullSpec: Unable to guess the opposite of \"" + file.checkval + "\"", COLOR_RED);
                #endif

            }
            else res = file.checkval;
...
bool FFQFullSpec::GetCommandLine(FULLSPEC_FILE &file, wxString &cmd, wxUniChar command_quote, wxUniChar value_quote)
{
    ...
    for (size_t i = 0; i < file.fields.size(); i++)
    {
        ...
        FULLSPEC_FIELD &field = file.fields[i];
        if ((field.type == ftHEADER) || (!field.is_enabled()) || (field.is_checked() == field.checked)) continue;

        //Get the value from the field, set to checkbox value if no control
        wxString val = GetFieldValue(file, field); //(field.ctrl != nullptr) ? GetFieldValue(file, field) : file.checkval;
        ...
        else
        {
            ...
            if (val.Len() > 0)
            {
                ...
                if ((!composite) && (value_quote != 0) && (val.Find(SPACE) >= 0)) cmd += value_quote + val + value_quote;
                else cmd += val;
...
void FFQFullSpec::Initialize(wxWindow *wnd)
{
    ...
    spec = DecompressString((unsigned char*)STR_AOMAV1, STR_AOMAV1_SIZE);
    FullSpecFileFromString(spec);

    spec = DecompressString((unsigned char*)STR_RAV1E, STR_RAV1E_SIZE);
    FullSpecFileFromString(spec);
...
void FFQFullSpec::MakeControlFor(FULLSPEC_FILE &file, FULLSPEC_FIELD &field, wxWindow *parent, wxSizer *sizer)
{
    ...
    else
    {
        ...
        if (field.require.Len() > 0) tt += ", require: " + field.require;

        #ifdef DEBUG
        tt += ", checked: " + (field.checked ? STR_YES : STR_NO);
        #endif // DEBUG
        ...
        if ((field.type == ftCHECK) || (field.require != EXCLAM))
        {
            ...
            field.check->SetValue(field.checked);
    ...
    do
    {
        ...
        if (val.Len() > 0)
        {
            ...
            else
            {
                ...
                if (StrSplit(val, key, EQUAL))
                {
                    ...
                    {
                        text = val;
                        //if (type == ftHEADER) type = ftCHECK; //The default field type
                    }
                    else if (key == "type")
                    {
        ...
        else if (name.Len() > 0)
        {
            ...
            if (idx >= 0)
            {
                ...
                //Update the default state for checkbox
                if (field.type == ftCHECK) field.checked = (field.def == file.checkval);

            }
            else if (text.Len() > 0) // ((text.Len() > 0) && (type != ftHEADER))
            {

                //Creating a new field
                if (type == ftHEADER) type = ftCHECK;
                /*if (type == ftCHECK)
                {
                    FFQConsole::Get()->AppendLine("check=\"" + name + "\" def=\"" + def + "\"", COLOR_BLUE);
                }*/
                file.fields.push_back(FULLSPEC_FIELD(type, name, text, value, range, def, require, hide == STR_YES, (type == ftCHECK) && (def == file.checkval)));
...
void FFQFullSpec::ResetControls(FULLSPEC_FILE &file)
{
    ...
    for (size_t i = 0; i < file.fields.size(); i++)
    {
        ...
        else if (field.type == ftCHECK) field.check->SetValue(field.checked);
        else
        {

            if (field.check != nullptr) field.check->SetValue(false);

            if (field.type == ftCHOICE)
            {
                wxChoice *ch = dynamic_cast<wxChoice*>(field.ctrl);
                ch->SetSelection(field.defidx);
            }
            ...
            {
                wxComboBox *cb = dynamic_cast<wxComboBox*>(field.ctrl);
                cb->Clear();
                cb->SetSelection(field.defidx);
            }
            ...
            {
                wxCheckListBox *cl = dynamic_cast<wxCheckListBox*>(field.ctrl);
                for (int ii = 0; ii < (int)cl->GetCount(); ii++) cl->Check(ii, (field.def == "*") || (ii == field.defidx));
            }

            else
            {
                wxTextCtrl *tx = dynamic_cast<wxTextCtrl*>(field.ctrl);
                if (tx != nullptr) tx->ChangeValue(field.def);
            }
    ...
    wxNotebook *nb = dynamic_cast<wxNotebook*>(file.evt_handler->GetParent());
    if (nb != nullptr) nb->SetSelection(0);
    file.evt_handler->UpdateControls();
...
void FFQFullSpec::SetCommandLine(FULLSPEC_FILE &file, wxString cmd)
{
    ...
    bool composite = (file.composite.Len() > 0) && cmd.StartsWith(file.composite);
    ...
    while (cmd.Len() > 0)
    {
        ...
        wxUniChar sep = 0;
        wxString cur = GetCmdToken(cmd, true, composite, &sep);
        ...
        if (idx >= 0)
        {
            ...
            if (field.type == ftCHECK)
            {
                ...
                field.check->SetValue(field.checked);
                wxString val = (sep == EQUAL) ? GetCmdToken(cmd, false, composite) : "";

                if (file.checkval.Len() > 0) field.check->SetValue(val == file.checkval);
                else if (val.Len() == 0) field.check->SetValue(true);
                else for (uint8_t i = 0; i < NUM_BOOLEANS; i++)
                {
                    if (val == BOOLEANS[i][0])
                    {
                        field.check->SetValue(false);
                        break;
                    }
                    else if (val == BOOLEANS[i][1])
                    {
            ...
            else
            {
                ...
                wxString val = (field.name == QUESTION) ? cur : GetCmdToken(cmd, false, composite);
...
FFQFullSpec::FFQFullSpec(wxWindow* parent, wxWindowID id)
{
	...
	ResetBtn = new wxButton(this, ID_RESETBTN, FFQS(SID_COMMON_RESET));//, wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, _T("ID_RESETBTN"));
	BtnSizer->Add(ResetBtn, 1, wxALL|wxEXPAND, 5);
	BtnSizer->Add(-1,-1,1, wxALL|wxEXPAND, 5);
	BtnSizer->Add(-1,-1,1, wxALL|wxEXPAND, 5);
	OkBtn = new wxButton(this, ID_OKBTN, FFQS(SID_COMMON_OK));//, wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, _T("ID_OKBTN"));
	OkBtn->SetDefault();
	BtnSizer->Add(OkBtn, 1, wxALL|wxEXPAND, 5);
	CancelBtn = new wxButton(this, ID_CANCELBTN, FFQS(SID_COMMON_CANCEL));//, wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator, _T("ID_CANCELBTN"));
	...
	OkBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &FFQFullSpec::ActionClick, this);
	CancelBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &FFQFullSpec::ActionClick, this);
	ResetBtn->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &FFQFullSpec::ActionClick, this);
...
void FFQFullSpec::ActionClick(wxCommandEvent& event)
{
    ...
    else if ((evtId == ID_RESETBTN) && DoConfirm(FFQS(SID_RESET_TO_DEFAULT))) ResetControls(*m_File);
